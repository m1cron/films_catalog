...
public class FilmRestControllerV1 implements ApiFilm {
    ...
    @Override
    public FilmDTO getFilmById(@Min(1) long id) {
    ...
    @Override
    public FilmDTO editFilm(FilmDTO film) {
        filmService.save(filmMapper.toEntity(film));
        return film;
    }
    ...
    @Override
    public String deleteFilm(@Min(1) long id) {
        Film film = filmService.findById(id);
        if (film == null) {
            throw new NoSuchEntityException(String.format("There is no film with ID = %d in Database", id));
        }
